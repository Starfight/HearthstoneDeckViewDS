...
async def get_rank_chart(data: list) -> Image:
    ...
    # Create chart
    plt.plot(dates, ranks, 'o', markersize=10, linestyle='-', color="#FF9100")

    # Configure style
    plt.xlabel('Jour', fontdict={'fontsize': 20})
    date_formatter = mdates.DateFormatter('%d')
    plt.gca().xaxis.set_major_formatter(date_formatter)
    plt.gca().tick_params(axis='x', labelsize=16)
    plt.gca().xaxis.set_major_locator(mdates.DayLocator())
    plt.ylabel('Classement', fontdict={'fontsize': 20})
    plt.gca().tick_params(axis='y', labelsize=16)
    plt.gca().yaxis.set_major_locator(ticker.MultipleLocator(1))
    plt.gca().invert_yaxis()
    plt.gca().set_facecolor("none")
    plt.gca().set_alpha(0)
    plt.title('Évolution du classement', fontdict={'fontsize': 24})
